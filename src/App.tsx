...
function App() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // PERBAIKAN: Menggunakan template literal agar variabel API_URL terbaca
      ...
      toast.error("ID Login atau Password salah!", { id: loadingToast });
    }
  };
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get(`${API_URL}/items`, getAuthHeader());
      setItems(res.data);
  ...
  const fetchLogs = async () => {
    try {
      const res = await axios.get(`${API_URL}/logs`, getAuthHeader());
      setLogs(res.data);
    } catch (err) { console.error(err); }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && editId) {
        await axios.put(`${API_URL}/items/${editId}`, data, getAuthHeader());
        toast.success("Barang diperbarui");
      } else {
        await axios.post(`${API_URL}/items`, data, getAuthHeader());
  ...
  const executeDelete = async () => {
    if (itemToDelete) {
      try {
        await axios.delete(`${API_URL}/items/${itemToDelete}`, getAuthHeader());
        fetchItems(); fetchLogs();
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 text-slate-900">
        ...
        <div className="bg-white w-full max-w-md rounded-3xl overflow-hidden border border-slate-100 shadow-[0_25px_50px_-12px_rgba(79,70,229,0.3)]">
          <div className="bg-indigo-600 p-10 text-center text-white">
            ...
            <p className="text-indigo-100 text-sm mt-1">Management Gudang Profesional</p>
          </div>
          <form onSubmit={handleLogin} className="p-10 space-y-6">
            <input type="text" required className="w-full px-4 py-3 bg-slate-50 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Username" value={loginUsername} onChange={(e) => setLoginUsername(e.target.value)} />
            <div className="relative">
              <input type={showPassword ? "text" : "password"} required className="w-full px-4 py-3 bg-slate-50 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} />
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-10">
      ...
      <nav className="bg-white border-b px-8 py-4 sticky top-0 z-50 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-6">
            <div className="bg-indigo-600 p-2 rounded-lg text-white shadow-lg"><Package size={24} /></div>
            <span className="text-2xl font-bold tracking-tight text-slate-800">VENTO</span>
        </div>
        <div className="flex items-center gap-6">
          ...
          <button onClick={() => setShowLogoutModal(true)} className="text-red-500 font-bold flex items-center gap-2 border px-4 py-2 rounded-xl hover:bg-red-50 transition-all active:scale-95 shadow-sm"><LogOut size={18} /> Keluar</button>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto p-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10 text-slate-900">
          <div className="bg-white p-6 rounded-2xl border shadow-sm flex items-center gap-4">
            ...
            <div><p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Total Jenis Barang</p><h3 className="text-xl font-bold">{items.length} Item</h3></div>
          </div>
          <div className="bg-white p-6 rounded-2xl border shadow-sm flex items-center gap-4">
            <div className="bg-emerald-100 p-3 rounded-xl text-emerald-600"><DollarSign /></div>
            <div><p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Nilai Persediaan</p><h3 className="text-xl font-bold">Rp {totalAset.toLocaleString()}</h3></div>
          </div>
          <div className={`bg-white p-6 rounded-2xl border shadow-sm flex items-center gap-4 border-l-4 ${lowStockCount > 0 ? 'border-l-red-500' : 'border-l-slate-100'}`}>
            ...
            <div><p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Stok Kritis</p><h3 className={`text-xl font-bold ${lowStockCount > 0 ? 'text-red-600' : ''}`}>{lowStockCount} Item</h3></div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-slate-900">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <button onClick={exportToExcel} className="bg-emerald-600 text-white px-6 py-3.5 rounded-2xl font-bold shadow-lg flex items-center gap-2 active:scale-95 transition-all"><Download size={20}/> Excel</button>
            </div>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50 transition-all group text-slate-900">
                      <td className="px-6 py-5">
                        ...
                        <div className="text-xs font-bold mt-1">
                            ...
                            <span className={item.stok <= 5 ? 'text-red-500 font-black' : 'text-indigo-500'}>Stok: {item.stok} {item.satuan}</span>
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 text-slate-900">
              <h3 className="font-bold mb-6 flex items-center gap-2 text-slate-800"><Clock size={20} className="text-indigo-500" /> Riwayat Aktivitas</h3>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 backdrop-blur-md bg-slate-900/40">
          <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-10 text-center animate-in zoom-in-95 duration-300">
            ...
            <div className="flex flex-col gap-3 mt-8">
              ...
              <button onClick={() => setShowLogoutModal(false)} className="w-full bg-slate-100 text-slate-500 font-bold py-4 rounded-2xl transition-all">Batal</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* --- MODAL DELETE --- */}
      {showDeleteModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 backdrop-blur-md bg-slate-900/40 text-slate-900">
          <div className="bg-white w-full max-w-xs rounded-[2rem] shadow-2xl p-8 text-center border border-slate-100 animate-in zoom-in-95 duration-300">
            ...
            <div className="flex gap-3 text-slate-900">
              <button onClick={() => setShowDeleteModal(false)} className="flex-1 bg-slate-100 text-slate-500 font-bold py-3 rounded-2xl transition-all">Batal</button>
              <button onClick={executeDelete} className="flex-1 bg-red-500 text-white font-bold py-3 rounded-2xl active:scale-95 shadow-lg transition-all">Hapus</button>
            </div>
          </div>
        </div>
      )}
